...
/**
 * The slip book, fetched on the server and handed over already filtered.
 *
 * The filters are read off the URL here, not out of the book's memory, so the
 * page the browser is sent is the page that was asked for — a link to "every
 * lorry still to load for Gaal Cargo" opens on those slips rather than on the
 ...
 * one: filtering, writing, amending and printing are all things done to the
...
export default async function LoadingSlipsPage({
  ...
  await queryClient
    .prefetchQuery({
      queryKey: loadingSlipKeys.page(company.slug, query),
      queryFn: () => fetchLoadingSlips(company.slug, query, auth),
    })
  ...
  return (
    <HydrationBoundary state={dehydrate(queryClient)}>
      <LoadingSlipRegister key={company.slug} query={query} />
    </HydrationBoundary>
  )
}
